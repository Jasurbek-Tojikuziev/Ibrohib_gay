...
public class TurretTester extends LinearOpMode {
    ...
    private boolean rawOverride = false; // Y = raw+, X = raw-
    ...
    public void runOpMode() {
        ...
        telemetry.addLine("Y: RAW +0.4 power (hardware test)");
        telemetry.addLine("X: RAW -0.4 power (hardware test)");
        telemetry.addLine();
        ...
        while (opModeIsActive()) {
            ...
            // Y / X: raw motor override — bypasses PIDF, tests hardware directly
            rawOverride = gamepad1.y || gamepad1.x;
            if (gamepad1.y) {
                turret.manualRotateRaw(0.4);
            } else if (gamepad1.x) {
                turret.manualRotateRaw(-0.4);
            } else if (!rawOverride) {
                // Joystick control (right stick X): target jumps ahead → PIDF holds
                double js = gamepad1.right_stick_x;
                if (Math.abs(js) > 0.2) {
                    double newTarget = turret.getCurrentAngle() + js * 90.0;
                    newTarget = Math.max(-135.0, Math.min(135.0, newTarget));
                    turret.setTargetAngle(newTarget);
                }
    ...
    private void displayTelemetry() {
        ...
        // Controls
        telemetry.addLine("--- CONTROLS ---");
        telemetry.addData("Right Stick X", "Manual (±135°)");
