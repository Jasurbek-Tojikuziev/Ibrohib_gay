...
public class TurretController {
    ...
    public void update() {
        ...
        // Dpad Up (gamepad1) - финализирует калибровку и включает auto-aim
        // Edge detection: only on first press, not while held (prevents cross-talk with dpad_right/left)
        boolean dpadUpPressed = gamepad1 != null && gamepad1.dpad_up && !prevDpadUp;
        prevDpadUp = gamepad1 != null && gamepad1.dpad_up;
        if (dpadUpPressed) {
            turret.resetEncoder();   // фиксируем текущую позицию как 0°
            autoAimEnabled = true;
            prevDpadLeft = false;
            prevDpadRight = false;
            return;
        ...
        if (gamepad1 != null && gamepad1.dpad_left) {
            ...
            turret.resetEncoder();
            autoAimEnabled = false; // defensive: prevent dpad_up cross-talk from re-enabling
            prevDpadLeft = false;
            return;
        }
